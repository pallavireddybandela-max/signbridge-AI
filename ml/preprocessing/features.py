"""
Landmark Feature Normalization & Engineering for Indian Sign Language (ISL) Recognition.
Converts 21 raw MediaPipe (x, y, z) landmarks into scale-invariant, translation-invariant feature vectors.
"""
...
def calculate_angle_3d(p1: np.ndarray, p2: np.ndarray, p3: np.ndarray) -> float:
    """Calculate the cosine angle between vectors p2->p1 and p2->p3."""
    v1 = p1 - p2
    v2 = p3 - p2
...
def extract_landmark_features(landmarks: Union[List[Dict[str, float]], np.ndarray, List[List[float]]]) -> np.ndarray:
    """
    Extract translation-invariant and scale-invariant 83-dimensional feature vector from 21 MediaPipe hand landmarks.
    
    Input:
        landmarks: List of 21 landmark points [{x, y, z}, ...] or numpy array (21, 3)
    
    Output:
        83-dimensional numpy feature vector:
        - 63 normalized landmark coords (relative to wrist and scaled by middle-MCP distance)
        - 10 tip-to-origin & tip-to-palm distances
        - 5 adjacent fingertip Euclidean distances
        - 5 finger flexion joint angles
    """
    ...
    if palm_scale < 1e-6:
        # Fallback to max bounding box extent if wrist and middle MCP overlap
    ...
    # 3. Tip distances relative to wrist & palm center (MCP 9)
    tips = [4, 8, 12, 16, 20]
    ...
    # Concatenate all features: 63 + 10 + 5 + 5 = 83 features
...
def normalize_multi_hand_landmarks(multi_hand_landmarks: List[List[Dict[str, float]]]) -> np.ndarray:
    """Extract features from primary hand and append secondary hand features if present."""
    if not multi_hand_landmarks or len(multi_hand_landmarks) == 0:
        return np.zeros(83, dtype=np.float32)
    
    primary = extract_landmark_features(multi_hand_landmarks[0])
    return primary
